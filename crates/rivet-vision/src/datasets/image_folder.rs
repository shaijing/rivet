...
use rivet_data::errors::{DataError, DataResult, invalid_argument};
...
impl ImageFolderDatasetCore {
    pub fn new(root: PathBuf) -> DataResult<Self> {
        if !root.is_dir() {
...
impl ImageFolderDatasetCore {
    fn get_one(&self, index: usize) -> DataResult<EncodedImageSample> {
        let sample = self.samples.get(index).ok_or(DataError::IndexOutOfRange {
            index,
...
fn direct_child_dirs(root: &Path) -> DataResult<Vec<String>> {
...
fn image_paths_under(root: &Path) -> DataResult<Vec<PathBuf>> {
