...
impl<'a> ArrowRow<'a> {
    ...
    fn column(&self, column: &str) -> DataResult<&arrow::array::ArrayRef> {
        self.batch
    ...
    pub fn binary(&self, column: &str) -> DataResult<Buffer> {
    ...
    pub fn struct_binary(&self, struct_column: &str, field: &str) -> DataResult<Buffer> {
    ...
    /// LargeStringArray).
    pub fn utf8(&self, column: &str) -> DataResult<Buffer> {
        let array = self.column(column)?;
...
fn null_value(column: &str, row: usize) -> DataError {
...
mod tests {
    ...
    fn build() -> RecordBatch {
        ...
        let raw: BinaryArray = [Some(&b"a"[..]), Some(&b"bc"[..]), Some(&b""[..])]
            .into_iter()
            .collect();
        let text: StringArray = [Some("hello"), Some(""), Some("wörld")]
            .into_iter()
            .collect();
    ...
    fn nulls_and_optional_labels() {
        let batch = build();
        assert!(
            ArrowRow::new(&batch, 2)
                .struct_binary("img", "bytes")
                .is_err()
        );
        assert_eq!(ArrowRow::new(&batch, 1).optional_i64("opt").unwrap(), None);
        assert!(ArrowRow::new(&batch, 1).i64("opt").is_err());
        assert_eq!(
            ArrowRow::new(&batch, 0).optional_i64("label").unwrap(),
            Some(7)
        );
    }
